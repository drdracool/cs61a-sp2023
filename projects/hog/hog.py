...
def tail_points(opponent_score):
    ...
    opponent_score = math.floor(opponent_score / 10)
...
# BEGIN PROBLEM 4
def perfect_square(player_score):
    end = math.floor(player_score / 2) + 1
    count = 1
    while count <= end:
        if count * count == player_score:
            return True
        count += 1
    return False
...
def next_perfect_square(score):
    return (math.floor(math.sqrt(score) + 1)) ** 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score0 = 0
    score1 = 0
    cur = strategy(score0, score1)
    while score0 <= 100:
        while score1 <= 100:
            if strategy(score0, score1) != cur or strategy(score1, score0) != cur:
                return False
            score1 += 1
        score0 += 1
        score1 = 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max = 0
    for i in range(1, 11):
        cur = make_averaged(roll_dice, total_samples)(i, dice)
        if cur > max:
            max = cur
            result = i
    return result
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if (
        tail_points(opponent_score) > threshold
        or square_update(0, score, opponent_score) - score > threshold
    ):
        return 0
