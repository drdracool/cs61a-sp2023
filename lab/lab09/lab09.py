...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        branch = t.branches[0]
        return is_bst(branch) and (
            bst_max(branch) <= t.label or bst_min(branch) > t.label
        )
    if len(t.branches) == 2:
        ...
        return (
            is_bst(left)
            and is_bst(right)
            and bst_max(left) <= t.label
            and bst_min(right) > t.label
        )
    return False
...
def add_trees(t1, t2):
    ...
    return Tree(
        new_label, [add_trees(b1, b2) for b1, b2 in zip(t1_branches, t2_branches)]
    )
